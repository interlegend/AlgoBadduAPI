...
class PaperOrderManager:
    ...
    def create_paper_order(self, signal, option_price, option_atr, strike, signal_time):
        ...
        if self.position_tracker.get_open_position_count() >= 1:
            # logger.warning(f"⚠️  Signal for {signal} ignored, a position is already open.")
            return None
        
        # Calculate entry levels using strategy
        levels = self.strategy.calculate_entry_levels(signal, option_price, option_atr)
        
        # Use the historical timestamp from the signal
    ...
    def update_positions(self, ce_price, pe_price, ce_high, pe_high, nifty_indicators, current_candle_time):
        """
        Update all open positions with current prices and check for exits.
        The timestamp of the current candle is now required.
        """
        closed_orders = []
        ...
        for position in self.position_tracker.get_all_open_positions():
            ...
            if self.position_tracker.check_tp1_hit(order_id, current_high):
                self.trade_logger.log_event('TP1_HIT', f"TP1 hit for {side} {position['strike']}", {'order_id': order_id, 'new_sl': 'N/A'})
    ...
    def _check_exit_conditions(self, position, current_price, nifty_indicators, current_candle_time):
        """Check all exit conditions for a position using the historical candle time."""
        side = position['side']
        ...
        if self.strategy.check_macd_ema_exit(
            ...
            ema=nifty_indicators.get('ema21', 0),
            macd_hist=nifty_indicators.get('macd_hist', 0)
        ):
            return ("MACD/EMA Exit", current_price)
        
        # Use historical time for EOD check
        if self.strategy.check_eod_exit(current_candle_time.time()):
