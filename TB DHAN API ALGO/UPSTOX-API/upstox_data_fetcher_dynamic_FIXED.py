"""
UPSTOX HYBRID DATA FETCHER - ULTRA INSTINCT EDITION 🔥
Fixed by Claude Sonnet 4.5

Key Improvements:
1. ✅ Flexible Goldilocks Rule: At least ONE premium in [50-175], BOTH > 10
2. ✅ Expiry Day Detection: Automatically lowers threshold to ₹30
3. ✅ Expanded Search Range: Now searches ±750 points (15 strikes)
4. ✅ Better Logging: Clear emoji-based status messages
5. ✅ Optimized Validation: Only fetches first candle for premium check

Features:
- Fetches EXPIRED option data using expired-instruments API
- Fetches CURRENT option data using Historical V3 API
- Supports 5-minute intervals for both
- Smart ATM strike selection with premium validation
"""
...
def find_and_validate_atm_strike(spot_price, contracts, access_token, target_date, expiry_date, is_expired_api, search_range=15):
    """
    🔥 ULTRA INSTINCT ATM FINDER 🔥

    Find the best ATM strike with the GOLDILOCKS RULE

    NEW FLEXIBLE LOGIC:
    - For NORMAL days: At least ONE premium in [50, 175], BOTH > 10
    - For EXPIRY days: At least ONE premium in [30, 175], BOTH > 10
    ...
    if is_expiry:
        print(f"[ATM-SEARCH] 🔥 EXPIRY DAY DETECTED - Using looser bounds [{lower_bound}, {upper_bound}]")
    else:
        print(f"[ATM-SEARCH] Normal day - Using standard bounds [{lower_bound}, {upper_bound}]")
    ...
    def validate_strike(current_strike):
        ...
        print(f"               Goldilocks: {'✅ PASS' if goldilocks_pass else '❌ FAIL'}")
...
def main():
    print("="*70)
    print("🔥 HYBRID UPSTOX DATA FETCHER - ULTRA INSTINCT EDITION")
    print("Fixed by Claude Sonnet 4.5")
    print("="*70)
    ...
    for day_num, target_date in enumerate(days_to_process, 1):
        ...
        # 🔥 THE FIX: Use the new smart validation function
    ...
    if all_option_data:
        ...
        output_file = 'atm_daily_options_HYBRID_V3_FIXED.csv'
