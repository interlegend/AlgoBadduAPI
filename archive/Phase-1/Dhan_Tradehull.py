	...
	def get_intraday_data(self,tradingsymbol,exchange,timeframe, debug="NO"):			
		try:
			...
			if tradingsymbol in self.commodity_step_dict.keys():
				...
				security_check = instrument_df[((instrument_df['SEM_TRADING_SYMBOL'].str.strip().str.upper() == tradingsymbol) | (instrument_df['SEM_CUSTOM_SYMBOL'].str.strip().str.upper() == tradingsymbol)) & (instrument_df['SEM_EXM_EXCH_ID'] == instrument_exchange[exchange])]
				if security_check.empty:
					raise Exception("Check the Tradingsymbol or Exchange")
				security_id = security_check.iloc[-1]['SEM_SMST_SECURITY_ID']

			instrument_type = instrument_df[((instrument_df['SEM_TRADING_SYMBOL'].str.strip().str.upper() == tradingsymbol)|(instrument_df['SEM_CUSTOM_SYMBOL'].str.strip().str.upper() == tradingsymbol))&(instrument_df['SEM_EXM_EXCH_ID']==instrument_exchange[exchange])].iloc[-1]['SEM_INSTRUMENT_NAME']
			time.sleep(2)
